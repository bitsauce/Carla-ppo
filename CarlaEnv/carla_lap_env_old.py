...
from hud import HUD
from draw_lap import compute_lap
...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000, viewer_res=(1280, 720), obs_res=(1280, 720),
        ...
        lap = compute_lap(self.world.map)
        self.lap_waypoints = [lap[i][0] for i in range(0, len(lap), 25)]

        self.previous_wp = self.get_closest_waypoint_on_lap(self.world.map.get_spawn_points()[self.spawn_point])

        # Reset env to set initial state
        self.reset()

    def get_closest_waypoint_on_lap(self, transform):
        location = transform.location

        closest_wp = (float("inf"), None)
        for wp in self.lap_waypoints:
    ...
    def reset(self, is_training=True):
        ...
        self.vehicle.tick()
        if is_training:
            if self.previous_wp:                       # First call to reset this is False
                self.vehicle.set_transform(self.previous_wp.transform) # Move vehicle 10 waypoints back
        else:
        ...
        return self.step(None, is_training)[0]
    ...
    def step(self, action, is_training=True):
        ...
        if action is not None:
            ...
            #for wp in self.lap_waypoints:
            #    self.world.debug.draw_point(wp.transform.location, life_time=1000.0)

            #self.world.debug.draw_point(self.world.map.get_spawn_points()[self.spawn_point].location, color=carla.Color(0, 0, 255), life_time=1000.0)
        ...
        loc, wp_loc = vector(self.closest_waypoint.transform.location), vector(transform.location)
        ...
        lap_wp = self.get_closest_waypoint_on_lap(transform)
        if is_training:
            dot = np.dot(vector(lap_wp.transform.get_forward_vector())[:2],
                         vector(transform.location - lap_wp.transform.location)[:2])
            if dot > 0.0: # Have we passed the point?
                self.previous_wp = lap_wp
        else:
            if self.distance_traveled > 1500:#self.lap_wp_index > len(self.lap_waypoints):
                self.terminal_state = True
            #lap_wp == self.lap_waypoints[0]
        #self.world.debug.draw_point(self.previous_wp_list[0].transform.location, color=carla.Color(0, 255, 0), life_time=1.0)
...
if __name__ == "__main__":
    ...
    env = CarlaEnv(obs_res=(160, 80), spawn_point=1)#0)
